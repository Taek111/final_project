...
def send_to_appUser(recv_num,type):

    # set api key, api secret
    api_key = "NCSCD9BOHCK6VQWE"
    api_secret = "FJD0BIWIBV5CPF6Q1Q1VNNI9RXTLWITM"
    ...
    if type == 1:
        params['text'] = '사용자의 실내활동이 감지되지 않습니다. 조치를 취한 후, "세이프 케어"앱을 실행해주세요.' # Message
    elif type == 2:
        params['text'] = '독거인에게서 응급구조요청이 들어왔습니다. 조치를 취한 후, "세이프 케어"앱을 실행해주세요.' # Message
...
def send_to_119(type):
    ...
    params['from'] = '01082156604' # Sender number
    if type == 1:
        params['text'] = '독거인의 활동이 감지되지 않습니다. 조치 바랍니다.(주소: 경기도 수원시 장안구 일월로 91번길 19-2, 305호)' # Message
    elif type == 2:
        params['text'] = '독거인에게서 응급구조요청이 들어왔습니다. 조치 바랍니다. (주소: 경기도 수원시 장안구 일월로 91번길 19-2, 305호)' # Message
    # Optional parameters for your own needs. more informations visit to http://www.coolsms.co.kr/SMS_API_v2#POSTsend
    # params["image"] = "desert.jpg" # image for MMS. type must be set as "MMS"
    # params["mode"] = "test" # 'test' 모드. 실제로 발송되지 않으며 전송내역에 60 오류코드로 뜹니다. 차감된 캐쉬는 다음날 새벽에 충전 됩니다.
    # params["delay"] = "10" # 0~20사이의 값으로 전송지연 시간을 줄 수 있습니다.
